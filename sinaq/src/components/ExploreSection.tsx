...
// Helper to render the logos from the third screen style
    ...
    return (
      <div className="flex items-center gap-2">
        <div className="w-5 h-5 bg-white relative flex items-center justify-center overflow-hidden" style={{ clipPath: 'polygon(25% 0, 100% 0, 100% 100%, 0 100%, 0 25%)' }}>
           <div className="w-0 h-0 border-t-[6px] border-t-transparent border-r-[8px] border-r-[#030812] border-b-[6px] border-b-transparent -ml-1"></div>
        </div>
    ...
    return (
      <div className="flex items-center gap-1.5">
        <div className="w-5 h-5 border-[3px] border-[#65a30d] rounded-[2px] transform rotate-45 relative">
           <div className="absolute inset-0 m-auto w-1.5 h-1.5 bg-[#65a30d] rounded-full transform -rotate-45"></div>
        </div>
  ...
  return (
    <section className="bg-[#030812] pt-12 md:pt-16">
      <div className="container mx-auto px-4 md:px-12">
        ...
        {/* Pills container */}
        <div className="flex flex-wrap gap-4 mb-10 overflow-x-auto no-scrollbar">
          {PILLS.map(pill => {
            ...
            return (
              <button
                ...
                className={`whitespace-nowrap px-4 py-2 rounded-full text-[14px] font-medium transition-all ${
                  isActive 
                    ? 'bg-[#22c55e] text-white border-0 shadow-[0_0_15px_rgba(34,197,94,0.3)]' 
                    : 'bg-transparent text-gray-300 border border-slate-600 hover:border-white hover:text-white cursor-pointer'
                }`}
              >
                {pill}
              </button>
      ...
      <div className="container mx-auto px-4 md:px-12 pb-16 md:pb-20 overflow-hidden relative">
        <div className="flex gap-5 md:gap-6 overflow-x-auto pb-6 pt-2 snap-x snap-mandatory no-scrollbar -mx-4 px-4 md:mx-0 md:px-0">
          {visibleCards.map((card) => (
            <div 
              key={card.id} 
              className="flex-shrink-0 w-[280px] snap-start group cursor-pointer hover:-translate-y-2 transition-transform duration-300"
            >
              <div className="bg-[#0a1122] border border-[#1e293b] rounded-[16px] aspect-[16/11] mb-4 overflow-hidden relative shadow-lg">
                {/* Fallback pattern/color for the card hero since we don't have images. Making it transparent dark to match the theme. */}
                <div className="w-full h-full bg-gradient-to-br from-[#10213d] to-[#081121] opacity-70 group-hover:opacity-100 transition-opacity" />
              </div>
              ...
              <h3 className="text-[18px] font-bold text-white mb-1 leading-snug group-hover:text-[#22c55e] transition-colors">
                {card.title}
              </h3>
